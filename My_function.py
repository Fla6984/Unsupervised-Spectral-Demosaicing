...
def msfaTOcube(raw, msfa_size):
    mask = np.zeros((raw.shape[0], raw.shape[1], msfa_size**2), dtype=int)
    cube = np.zeros((raw.shape[0], raw.shape[1], msfa_size**2), dtype=int)
...
def load_img(filepath):
    """
    Loads a TIFF image using the tifffile library.

    Args:
        filepath (str): Path to the .tif file

    Returns:
        numpy.ndarray: Image array (H x W x C), assuming that’s how
                       your TIFF is stored. Adjust shape if needed.
    """
    img = tifffile.imread(filepath)  # returns a NumPy array
    # If your TIFF is (channels, height, width), you might want:
