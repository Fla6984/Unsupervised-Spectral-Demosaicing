...
def load_img(filepath):
    """
    Load a TIFF image and transpose channels.
    Args:
        filepath (str): Path to the TIFF file.
    Returns:
        numpy.ndarray: Transposed image array (Height, Width, Channels).
    """
    ...
    try:
        ...
        if img.ndim == 3:  # Assuming the input is (Bands, Height, Width)
            img = img.transpose(1, 2, 0)  # Transpose to (Height, Width, Bands)

        print(f"[DEBUG] Loaded and transposed image shape: {img.shape}")
        return img
    except Exception as e:
        print(f"[ERROR] Exception while loading image {filepath}: {e}")
        return None
...
model = Net()
m_state_dict = torch.load(opt.model, map_location=torch.device('cpu'))
model.load_state_dict(m_state_dict)
im_gt_y = load_img(opt.val_dir + "/" + opt.image + "_" + "IMECMine_D65" + ".tif")  # 512, 512, 16 shape
# PPID = load_img(join("G:\dataset\CAVE2\\validation_CAVE", opt.image, opt.image, "PPID_HA.tif"))  # 512, 512, 16 shape
...
ergas_predicted = ergas(im_gt_y.transpose(2, 1, 0), im_h_y.transpose(2, 1, 0))
